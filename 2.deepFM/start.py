...
fold = list(StratifiedKFold(n_splits = 4, shuffle = True, random_state = 2018).split(train_x, train_y))
...
for i, (train_idx, valid_idx) in enumerate(fold):
    ...
    deepfm = DeepFM(**param)
    deepfm.fit(train_xi, train_xv, train_y_, valid_xi, valid_xv, valid_y_)

print 'Fit all train data......'
deepfm = DeepFM(**param)
deepfm.fit(train_di, train_dv, train_y)
deepfm.predict(test_di, test_dv, test_y)
